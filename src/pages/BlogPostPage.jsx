...
export function BlogPostPage() {
  ...
  if (!post) return <div className="route-page empty-state"><h1>未找到该文章</h1><Link to="/blog">返回博客</Link></div>;
  ...
  return (
    <article className="article-page">
      ...
      <Link className="back-link" to="/blog"><ArrowLeft size={17} /> 返回博客</Link>
      ...
      <div className="article-layout">
        <aside className="article-toc"><span>文章目录</span>{post.headings.map((heading) => <a href={`#${heading.id}`} key={heading.id}>{heading.label}</a>)}</aside>
        ...
        <aside className="article-info"><span>主题标签</span><div className="tag-list">{post.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><p>本文来自我的 Obsidian Blog 知识库，记录当前阶段的研究、产品观察与工程实践。</p></aside>
      </div>
      <nav className="article-pagination">{previous ? <Link to={`/blog/${previous.slug}`}><ArrowLeft size={17} /><span>上一篇<strong>{previous.title}</strong></span></Link> : <span />}{next ? <Link to={`/blog/${next.slug}`}><span>下一篇<strong>{next.title}</strong></span><ArrowRight size={17} /></Link> : null}</nav>
    </article>
  );
}
